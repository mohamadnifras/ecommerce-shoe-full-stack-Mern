...
function KindsProduct() {
  ...
  useEffect(() => {
    dispatch(
      fetchProducts({ page: currentPage || 1, limit: 10, category: "kids" })
    );
  ...
  return (
    <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 pt-25">
      ...
      <img
        src="/kids1.png"
        alt="kidss Collection"
        className="w-full md:h-full object-cover rounded-md shadow-md hidden md:block"
      />
      <img
        src="/Kids.avif"
        alt="kids Collection"
        className="w-full md:h-full object-cover rounded-md shadow-md block md:hidden"
      />
    </div>
      {loading ? (
